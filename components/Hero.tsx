...
const HERO_IMAGES = [
  {
    src: "https://kimi-web-img.moonshot.cn/img/images.unsplash.com/898f615d60f7067119ea660a0e29af667f512c80",
    alt: "Dal Lake with traditional houseboats in Srinagar, Kashmir",
  },
  {
    src: "https://kimi-web-img.moonshot.cn/img/images.unsplash.com/102fc424abe089958cb63d1f2bbc4c0dc5d875a9",
    alt: "Snow-covered ski slopes and mountains in Gulmarg, Kashmir",
  },
  {
    src: "https://kimi-web-img.moonshot.cn/img/images.unsplash.com/d56c979d853fb5e6ce473f399f23c7003846cf08",
    alt: "Lush green meadows and pine forests in Pahalgam, Kashmir",
  },
];
...
const slides = [
  {
    ...
    sub: "Experience Kashmir's breathtaking landscapes — serene lakes, snow-capped peaks, and lush meadows await you.",
    badge: "🏔️  Srinagar · Kashmir",
  },
  {
    title: "Adventure Awaits in",
    highlight: "Gulmarg",
    sub: "Asia's highest gondola, world-class skiing, and meadows blanketed in wildflowers.",
    badge: "⛷️  Gulmarg · Baramulla",
  },
  {
    title: "Romance Blooms in",
    highlight: "Pahalgam",
    sub: "Pine-forested valleys, sparkling rivers, and golden sunsets — the Valley of Shepherds.",
    badge: "🌸  Pahalgam · Anantnag",
  },
];
...
const DESTINATIONS = [
  {
    ...
    label: "Dal Lake",
    sub: "Srinagar",
    img: "https://images.unsplash.com/photo-1585409677983-0f6c41ca9c3b",
    emoji: "🛶",
  },
  {
    id: "gulmarg",
...
function getUpcomingMonths(count: number) {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      isPeak: PEAK_MONTHS.includes(d.getMonth()),
    };
  });
}
...
export default function Hero() {
  ...
  const [selTravType, setSelTravType] = useState<(typeof TRAVELER_TYPES)[0]>(
    TRAVELER_TYPES[1]
  );
  ...
  return (
    ...
    >
      ...
      >
        // Only render the ACTIVE slide's image, not all 3
        {HERO_IMAGES.map(
          (imgData, i) =>
            slide === i && ( // ← Only render current slide!
              <motion.div
                key={imgData.src}
      ...
      >
        ...
        >
          <a
            href="#packages"
            className="btn-gold px-7 py-3.5 rounded-full font-bold text-[#1B4332] text-sm sm:text-base shadow-xl text-center w-full sm:w-auto"
          >
            Explore Packages
          </a>
          <a
            href="#destinations"
            className="px-7 py-3.5 rounded-full font-bold text-white text-sm sm:text-base border-2 border-white/50 hover:bg-white/15 transition-all text-center w-full sm:w-auto"
          >
            View Destinations
          </a>
        </motion.div>
          ...
          <div className="bg-[#0a1612]/80 backdrop-blur-2xl border border-white/20 rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-[0_8px_64px_rgba(0,0,0,0.7)] ring-1 ring-white/5">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3">
              <button
                ...
                className={`relative flex flex-col items-start gap-0.5 px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl border transition-all duration-200 text-left cursor-pointer ${
                  open === "dest"
                    ? "bg-white/25 border-[#C9A84C]"
                    : "bg-white/12 border-white/20 hover:bg-white/22 hover:border-[#C9A84C]/50"
                }`}
              >
                <div className="flex items-center gap-1.5 w-full">
                  <MapPin size={12} className="text-[#C9A84C] shrink-0" />
                  <span className="text-[#C9A84C]/90 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">
                    Where
                  </span>
                  {selDest && (
                    <Check
                      size={9}
                      className="text-[#C9A84C] ml-auto shrink-0"
                      strokeWidth={3}
                    />
                  )}
                </div>
                <p
                  className={`text-xs sm:text-sm font-semibold truncate w-full ${
                    selDest ? "text-white" : "text-white/70"
                  }`}
                >
                  {selDest ? selDest.label : "Destination"}
                </p>
                {selDest && (
                  <p className="text-[#C9A84C] text-[10px] leading-none">
                    {selDest.sub}
                  </p>
                )}
              </button>
              ...
              >
                ...
                <p
                  className={`text-xs sm:text-sm font-semibold truncate w-full ${
                    selMonth ? "text-white" : "text-white/70"
                  }`}
                >
                  {selMonth
                    ? `${selMonth.label} ${selMonth.year}`
              ...
              <button
                onClick={() =>
                  setOpen(open === "travelers" ? null : "travelers")
                }
                className={`relative flex flex-col items-start gap-0.5 px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl border transition-all duration-200 text-left cursor-pointer ${
                  open === "travelers"
                    ? "bg-white/25 border-[#C9A84C]"
              ...
              >
                ...
                <p className="text-xs sm:text-sm font-semibold text-white truncate w-full">
                  {travelersLabel()}
                </p>
                <p className="text-[#C9A84C] text-[10px] leading-none">
                  {selTravType.icon} {selTravType.label}
                </p>
              </button>
            </div>
            ...
            <AnimatePresence>
              ...
              {open === "date" && (
                <motion.div
                  ...
                  className="overflow-hidden mb-3"
                >
                  <p className="text-white/45 text-[10px] text-center mb-2 uppercase tracking-widest">
                    ...
                    {upcomingMonths.map((m) => (
                      <motion.button
                        ...
                        className={`relative rounded-xl py-2.5 px-2 text-center border transition-all duration-200 cursor-pointer ${
                          selMonth?.month === m.month &&
                          selMonth?.year === m.year
                      ...
                      >
                        {m.isPeak && selMonth?.month !== m.month && (
                          <span className="absolute -top-1 -right-1 w-3 h-3 bg-rose-400 rounded-full text-[7px] flex items-center justify-center text-white font-bold">
                            ✨
                          </span>
                        )}
                ...
                >
                  ...
                  <div className="flex flex-col gap-3 justify-center">
                    {[
                      ...
                      >
                        <span className="text-white/60 text-xs font-semibold w-14">
                          {c.label}
                        </span>
                        <button
                        ...
                        <button
                          onClick={() => c.set(Math.min(c.max, c.value + 1))}
                          className="w-7 h-7 rounded-full bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition-colors cursor-pointer font-bold text-lg leading-none"
                        >
                          +
                        </button>
            ...
            >
              ...
              {plannerComplete ? (
                ...
                <>
                  <span className="sm:hidden">Select destination first</span>
                  <span className="hidden sm:inline">
                    Select destination &amp; month to continue
                  </span>
                </>
              )}
            </motion.button>
      ...
      >
        <a
          href="#stats"
          className="animate-bounce-slow flex flex-col items-center text-white/60 hover:text-white transition-colors"
        >
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 pointer-events-none">
        ...
        >
          <path
            d="M0 60V30C360 0 720 60 1080 30C1260 15 1380 40 1440 30V60H0Z"
            fill="white"
          />
        </svg>
      </div>
